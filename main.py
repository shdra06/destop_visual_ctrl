...
MOVEMENT_THRESHOLD = 0.005 # High Sensitivity for Volume (0.5%)
SMOOTHING_BUFFER = 5
...
def main():
    ...
    with vision.HandLandmarker.create_from_options(options) as landmarker:
        ...
        # cap = cv2.VideoCapture(0) # Old slow method
                        ...
                        if current_gesture_state == VOL_ID:
                            ...
                            if prev_y is not None:
                                ...
                                if speed > MOVEMENT_THRESHOLD:
                                    if time.time() - last_volume_update > 0.1: # 10Hz Update Rate
                                        
                                        # Refined Sensitivity Logic
                                        # speed = movement ratio (0.0 to 1.0)
                                        # 0.005 = 0.5% screen height movement
                                        
                                        if speed < 0.02:
                                            steps = 2  # ~4% Volume change (Fine Control)
                                            speed_text = "FINE"
                                        elif speed < 0.05:
                                            steps = 5  # ~10% Volume change (Medium)
                                            speed_text = "MED"
                                        else:
                                            steps = 10 # ~20% Volume change (Fast)
                                            speed_text = "FAST"
